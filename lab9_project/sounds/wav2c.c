...
// Leave the following line uncommented unless you want to generate a simple
// tone.
...
#define SUPPORTED_WAVE_DATA_BIT_SIZE                                           \
  16 // Program can only handle this size of data for now.
...
typedef struct {
  ...
  uint32_t chunkSize; // Size of the entire file minus 8 bytes (chunkSize +
                      // chunkSize).
  char format[FORMAT_SIZE];      // Should contain "WAVE". Add room for NULL.
  char subchunk1Id[FORMAT_SIZE]; // Should contain the letters "fmt ". Add room
                                 // for NULL.
  ...
  char subchunk2Id[SUBCHUNK2ID_SIZE]; // Should contain "data". Add room for
                                      // NULL.
  uint32_t
      subchunk2Size; // Number of remaining bytes in the file (the actual data).
} waveFileHeader_t;
...
// Returns true if the header looks OK by inspecting various fixed-value fields,
// false otherwise. Prints out informational messages if the os (output stream)
...
const char *get_filename_extension(const char *fileName) {
  ...
  if (!dot || dot == fileName)
    return ""; // If "." doesn't exist or if file name starts with ".", return
               // empty string.
  return dot + 1; // Advance to the string that follows "."
}

int main(int argc, char *argv[]) {
  // Print a helpful error message and exit if a file-name was not provided on
  // the command line.
  if (argc != 2) {
  ...
  strncpy(inputFileName, argv[1],
          MAX_FILENAME_LENGTH); // Copy the argv[1] filename into the working
                                // buffer.
  // Make sure that the file-name has a .wav suffix.
  const char *extension =
      get_filename_extension(inputFileName); // Get the suffix.
  // Compare the suffix and generate an error message if it is incorrect.
  if (strncmp(extension, WAV_SUFFIX, MAX_FILENAME_LENGTH)) {
    fprintf(stderr,
            "ERROR: input file-name \"%s\" does not have a %s suffix.\n",
  ...
  if (!waveFileHeaderOk(stderr, &header)) { // Check the header.
    fprintf(stderr,
            "ERROR: Input file (%s) contains errors. Maybe not a .wav file? "
            "See proceeding messages for details.\n",
            inputFileName);
    printWaveFileHeader(stderr, &header);
    exit(-1);
  }
  // File contents look good if you get this far.
  // Only handle mono files for now, check for this and exit if the file is not
  // mono.
  if (header.numChannels > 1) {
    fprintf(stderr, "INFO: wave file contains stereo data. This program only "
                    "handles monophonic data for now. Exiting...\n");
    exit(-1);
  }
  // Only handle 16-bit data for now.
  if (header.bitsPerSample != SUPPORTED_WAVE_DATA_BIT_SIZE) {
    fprintf(stderr,
            "INFO: wave file contains %d-bit data. This program can only "
            "process %d-bit data. Exiting...\n",
            header.bitsPerSample, SUPPORTED_WAVE_DATA_BIT_SIZE);
    exit(-1);
  }
  printWaveFileHeader(stderr, &header);
  // Everything looks good. Go ahead and generate the .h and .c files. Exit with
  // an error if either file already exists. First, open the .h file and output
  // the necessary declarations. Then, close the .h file.
  char hFileName[MAX_FILENAME_LENGTH]; // .h file-name.
  strncpy(hFileName, inputFileName,
          MAX_FILENAME_LENGTH); // Basename is just the input-file-name.
  strncat(hFileName, H_FILE_SUFFIX, MAX_FILENAME_LENGTH); // Add the suffix.
  char cFileName[MAX_FILENAME_LENGTH];                    // .c file-name.
  strncpy(cFileName, inputFileName,
          MAX_FILENAME_LENGTH); // Basename is just the input-file-name.
  ...
  // If you get this far, you can go ahead and open the .c and .h files for
  // write.
  ...
  fprintf(hFileFp,
          "// This file was generated by executing this statement: wav2c %s\n",
          inputFileName);
  ...
  uint32_t arraySize =
      header.subchunk2Size / 2; // Wave file is counted by bytes, output file is
  ...
  // File pointer to the input file should be pointing at the first value after
  // the header is read, so just start from there.
  for (i = 0; i < header.subchunk2Size / 2; i++) {
    int16_t data; // Program only handles 16-bit PCM data for now. PCM data is
                  // signed.
    fread(&data, header.blockAlign, 1,
          inputFileFp); // Read 2 bytes into a signed 16-bit variable.
    uint16_t unsignedData =
        data + INT16_MAX; // Offset to unsigned for the sound CODEC.
